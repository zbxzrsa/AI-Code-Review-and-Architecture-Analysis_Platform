...
async def register(request: RegisterRequest, response: Response):
    ...
    # 1. Validate invitation code against database
    # 2. Check if email already exists
    # 3. Create user record with hashed password
    # Mock: Accept any invitation code for development
...
async def refresh_token(request: Request):
    ...
    """
    # Token refresh - production validates refresh token from database
...
async def get_current_user():
    ...
    """
    # Mock user - production extracts from JWT token
    return UserResponse(
        id="user_123",
...
async def change_password(request: ChangePasswordRequest):
    ...
    """
    # Production: Verify old password hash and update with new hash
...
async def forgot_password(request: ForgotPasswordRequest):
    ...
    Returns:
        Success message / 成功消息
    """
    # Production: Generate reset token, store in DB, send via email service
...
async def reset_password(request: ResetPasswordRequest):
    ...
    """
    # Production: Validate token from DB, check expiry, update password hash
