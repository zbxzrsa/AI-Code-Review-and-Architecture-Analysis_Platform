...
export function useCodeAnalysis() {
  ...
  const analyzeStandard = async (
    ...
    const response = await enhancedApi.post<AnalysisResult>(
      "/analysis/code",
      {
        code,
        ...options,
  ...
  const applyFix = useCallback(
    async (issue: AnalysisIssue, code: string): Promise<string | null> => {
      ...
      try {
        const response = await enhancedApi.post<{ fixedCode: string }>(
          "/analysis/fix",
          {
            issue,
            code,
  ...
  const applyAllFixes = useCallback(
    async (issues: AnalysisIssue[], code: string): Promise<string | null> => {
      ...
      try {
        const response = await enhancedApi.post<{
          fixedCode: string;
          fixedCount: number;
        }>("/analysis/fix-all", { issues: fixableIssues, code });
