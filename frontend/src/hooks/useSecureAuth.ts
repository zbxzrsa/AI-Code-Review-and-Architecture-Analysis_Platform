...
export function useSecureAuth() {
  ...
  const login = useCallback(async (credentials: LoginCredentials): Promise<LoginResult> => {
    const { email, password, rememberMe, invitationCode } = credentials;
    ...
    try {
      ...
      const response = await apiService.auth.login(email, password, invitationCode);
  ...
  }, [location, navigate, notify, setUser, setAuthenticated, setTwoFactorState, setLoading, setError]);

  /**
   * Verify two-factor authentication code
   */
  const verifyTwoFactor = useCallback(async (verification: TwoFactorVerification): Promise<LoginResult> => {
    const { code, isBackupCode } = verification;

    // Validate code format
    if (!isBackupCode && !twoFactorAuth.validateCodeFormat(code)) {
      return { success: false, error: 'Invalid code format. Enter 6 digits.' };
    }
    ...
    // Check rate limiting
    if (rateLimiter.shouldLimit('/auth/2fa/verify', rateLimiter.configs.twoFactor)) {
  ...
  }, [location, navigate, notify, setUser, setAuthenticated, setTwoFactorState, setLoading]);
  ...
  const register = useCallback(async (data: RegisterData): Promise<LoginResult> => {
    ...
    // Check rate limiting
    if (rateLimiter.shouldLimit('/auth/register', rateLimiter.configs.register)) {
  ...
  const requestPasswordReset = useCallback(async (email: string): Promise<{ success: boolean; error?: string }> => {
    ...
    // Check rate limiting
    if (rateLimiter.shouldLimit('/auth/password/reset', rateLimiter.configs.passwordReset)) {
