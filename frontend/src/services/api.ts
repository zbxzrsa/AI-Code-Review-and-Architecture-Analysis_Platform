...
import { errorLoggingService, ErrorCategory } from './errorLogging';
...
export const apiService = {
  ...
  projects: {
     ...
     * Update webhook
     */
    updateWebhook: (id: string, webhookId: string, data: { url?: string; events?: string[]; is_active?: boolean }) =>
     ...
     * Create API key
     */
    createApiKey: (id: string, data: { name: string; permissions: string[]; expires_at?: string }) =>
     ...
     * Get API key usage
     */
    getApiKeyUsage: (id: string, keyId: string, params?: { start_date?: string; end_date?: string }) =>
  ...
  user: {
    ...
    verifyEmail: (token: string) =>
      api.post('/user/email/verify', { token }),
     ...
     */
    revokeAllSessions: () =>
      api.post('/user/sessions/revoke-all'),
    ...
    revokeApiKey: (keyId: string) =>
      api.delete(`/user/api-keys/${keyId}`),
     ...
     */
    disconnectSlack: () =>
      api.delete('/user/integrations/slack'),
     ...
     */
    disconnectTeams: () =>
      api.delete('/user/integrations/teams'),
  ...
  admin: {
    ...
    providers: {
       ...
       */
      getMetrics: (providerId: string, params?: {
      ...
      setFallbackOrder: (providerIds: string[]) =>
        api.put('/admin/providers/fallback-order', { provider_ids: providerIds }),
    },
